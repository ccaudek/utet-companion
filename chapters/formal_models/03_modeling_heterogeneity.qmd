...
3) Gerarchico → Condivisione informazione + iper-parametri → Dati scarsi/rumorosi per soggetto; inferenze di popolazione
...
A differenza del modello *sample-level*, analizzato in precedenza, che stima un unico insieme di parametri ($(\alpha, \beta)$) per l'intero campione, risultando così cieco rispetto alle differenze individuali, il modello a livello individuale (o *person-level*) introduce un importante avanzamento concettuale e metodologico. Questo approccio stima parametri distinti, $\alpha_i$ e $\beta_i$, per ogni singolo partecipante, riconoscendo esplicitamente l'eterogeneità presente nella popolazione.
...
Il *modello a livello individuale* (*person-level*) stima parametri distinti $(\alpha_i, \beta_i)$ per ogni partecipante $i$. In questa formulazione, ogni individuo possiede il proprio tasso di apprendimento $\alpha_i$ e la propria deriva motivazionale $\beta_i$, consentendo di mappare la variabilità inter-individuale nei processi di aggiornamento degli obiettivi e di identificare profili comportamentali distinti. Il parametro $\sigma$, che cattura la variabilità residua (rumore decisionale), rimane comune a tutti i soggetti in questa versione di base (estendibile a varianze soggetto-specifiche quando necessario).
...
1. **Riduce il potere statistico**, perché ogni stima usa solo una frazione dei dati totali.
...
Un sintomo tipico è la presenza, per molti soggetti, di *intervalli di credibilità molto ampi* o di *posterior multi-modali* su $\alpha_i$ e/o $\beta_i$: segnali di informazione locale insufficiente e, talvolta, di *non-identificabilità* in specifiche regioni dello spazio dei parametri.

È in questo contesto che i *modelli gerarchici* mostrano il loro valore, “prestando” informazione a livello di popolazione per **regolarizzare** le stime individuali ed evitare valori estremi non supportati dai dati. I modelli gerarchici bayesiani (o *multilevel*) *concettualizzano* i parametri individuali non come entità indipendenti, ma come realizzazioni di una **distribuzione di gruppo** sovraordinata. In pratica, si introduce un **pooling parziale**: i parametri soggetto-specifici vengono tirati verso la tendenza di popolazione nella misura in cui i loro dati sono poco informativi.
...
$$
dove $(\mu_\alpha,\tau_\alpha)$ e $(\mu_\beta,\tau_\beta)$ sono *iper-parametri* che descrivono la popolazione. Questa struttura consente di bilanciare in modo ottimale *idiosincrasie individuali* e *regolarità aggregate* (Kruschke & Vanpaemel, 2015; Turner et al., 2013; Vincent, 2016). Il risultato è una *regolarizzazione*: le stime estreme vengono attenuate (*shrinkage*) verso la media di popolazione, riducendo il peso di stime instabili o rumorose e producendo inferenze più *robuste* e *interpretabili*, specialmente con campioni piccoli o pochi trial per soggetto (Boehm et al., 2018; Rouder & Lu, 2005).
...
La funzione tanh fornisce una soluzione elegante a questo requisito, mappando in modo continuo e differenziabile qualsiasi valore reale (ad esempio, il valore di un parametro latente non vincolato) nell'intervallo (-1, 1). Il fattore moltiplicativo di 0.95 applicato all'output di tanh (cioè $\alpha = 0.95 \cdot \text{tanh}(\cdot)$) introduce un ulteriore margine di sicurezza numerica, ritirandosi leggermente dai limiti estremi dell'intervallo. Questa contrazione marginale si è rivelata efficace nel ridurre il verificarsi di divergenze durante il campionamento con la tecnica del Hamilton Monte Carlo (HMC), migliorando la stabilità e l'efficienza computazionale dell'inferenza.
