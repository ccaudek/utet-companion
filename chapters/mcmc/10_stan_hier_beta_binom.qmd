...
In molti contesti di ricerca psicologica, i dati sperimentali consistono in serie di successi e insuccessi registrati per ciascun partecipante su un numero definito di prove. Questo scenario solleva tre interrogativi fondamentali:

- **(Q1)** Qual è la probabilità di successo di ogni singolo individuo?
- **(Q2)** Come variano queste probabilità all'interno della popolazione, in termini di valore centrale e dispersione?
- **(Q3)** Data la limitatezza del numero di prove per partecipante, come è possibile evitare stime individuali estreme e instabili?

Un quadro di riferimento efficace per rispondere a queste domande è offerto dal *modello gerarchico beta-binomiale*. Questo approccio introduce un livello superiore (la "popolazione") che regolarizza le stime individuali attraverso un meccanismo di *shrinkage*, quantificando al contempo la variabilità tra i soggetti.

La struttura del modello si articola su tre livelli:

1.  **Livello dei Dati:** Le osservazioni di ciascun partecipante sono modellate come variabili casuali binomiali, dove il numero di successi dipende da una probabilità sottostante e da un numero fisso di prove.
2.  **Livello Individuale:** Le probabilità di successo dei singoli partecipanti non sono trattate come valori fissi, bensì come estrazioni da una distribuzione Beta comune all'intera popolazione.
3.  **Livello di Popolazione:** I parametri della distribuzione Beta, che ne definiscono la forma, sono a loro volta considerati incerti e vengono stimati dai dati, utilizzando distribuzioni a priori che riflettono l'incertezza del ricercatore.

Un'interpretazione particolarmente intuitiva degli iperparametri della Beta si ottiene utilizzando una *parametrizzazione in termini di media e concentrazione*. La media rappresenta la probabilità di successo tipica nella popolazione, mentre la concentrazione indica il grado di omogeneità tra gli individui. Alti valori di concentrazione suggeriscono che le probabilità individuali sono molto vicine alla media del gruppo; valori bassi, al contrario, segnalano una sostanziale eterogeneità. Questa riformulazione non solo facilita la scelta di distribuzioni a priori sensate, ma rende anche l'interpretazione dei risultati immediatamente pertinente e ricca di senso sul piano psicologico.

Dal punto di vista inferenziale, il modello fornisce stime simultanee a livello individuale e di popolazione. Le *stime individuali* beneficiano del fenomeno dello *shrinkage*: invece di essere calcolate in isolamento, le probabilità di ciascun partecipante vengono "attratte" verso la media di popolazione. L'entità di questa regolarizzazione è inversamente proporzionale alla quantità di dati disponibili per l'individuo, un meccanismo che previene efficacemente stime estreme e inaffidabili quando il numero di prove è esiguo. Le *stime di popolazione*, d'altro canto, offrono una sintesi della distribuzione delle abilità, permettendo ad esempio di stabilire se la prestazione media del campione sia credibilmente diversa da un benchmark teorico, come il puro caso.

In questo capitolo, illustreremo la costruzione e la stima di un modello gerarchico beta-binomiale utilizzando *Stan*, applicandolo a un set di dati reali tratto dalla letteratura. L'analisi mostrerà non solo l'implementazione tecnica del modello, ma anche come interpretare in modo integrato gli output, evidenziando il ruolo cruciale della struttura gerarchica nel gestire la variabilità intrinseca dei dati psicologici in modo coerente.
...
Se le diagnostiche di convergenza confermano un'adeguata esplorazione della distribuzione a posteriori, si può procedere con l'analisi sostantiva. In caso contrario, è opportuno valutare opportuni riparametrizzazioni o altri aggiustamenti del modello. Un aumento del parametro `adapt_delta` è generalmente efficace nell'eliminare le divergenze, sebbene a scapito di una velocità di campionamento leggermente ridotta. Qualora si riscontri un'elevata autocorrelazione, un incremento di `iter_sampling` consente di migliorare l'*Effective Sample Size* (ESS). Per problemi più ostinati, legati alla geometria della distribuzione a posteriori, è spesso vantaggioso ricorrere a una riparametrizzazione alternativa del modello. Nel caso specifico qui trattato, la parametrizzazione basata sulla media della popolazione ($\mu$) e sul parametro di concentrazione (κ) rappresenta una soluzione praticabile, in grado di regolarizzare la geometria dello spazio dei parametri, come si avrà modo di illustrare nella sezione dedicata all'implementazione.

Una volta conclusa questa fase diagnostica, il modello è pronto per l'esplorazione delle distribuzioni a posteriori. L'obiettivo primario diventa quindi collegare le quantità di interesse psicologico—le probabilità individuali $p_i$ e la media di popolazione—alle domande di ricerca che hanno motivato l'analisi. Nei prossimi paragrafi ci dedicheremo all'interpretazione delle distribuzioni a posteriori, illustreremo le modalità per visualizzare il fenomeno dello *shrinkage* e presenteremo metodi per quantificare l'evidenza a supporto o a sfavore dell'ipotesi che la prestazione sia riconducibile al puro caso.

### Controlli predittivi essenziali

**Verifica predittiva a priori.** Prima di includere i dati osservati, è necessario valutare se le distribuzioni a priori specificate per le coppie (α, β) o (μ, κ) generino una distribuzione plausibile delle probabilità individuali $p_i$ e dei conteggi predetti $y_i$. Un prior che assegni massa elevata a scenari non realistici—come prestazioni prossime a 0 o 1 in un contesto di alta concentrazione—va considerato incoerente con le conoscenze di dominio e deve essere pertanto rivisto.

**Verifica predittiva a posteriori.** Dopo la stima del modello, si simula la generazione di nuovi dati replicati basati sulla distribuzione a posteriori. La loro coerenza con i dati empirici osservati va valutata confrontando metriche chiave, quali la distribuzione delle proporzioni individuali, la media gruppale e la varianza tra i soggetti. Discrepanze sistematiche in questi confronti possono indicare una cattiva specificazione del parametro di concentrazione κ o una violazione dell'ipotesi di scambiabilità degli effetti individuali. 
