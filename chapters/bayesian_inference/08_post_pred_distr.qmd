...
```{r}
# Distribuzione iniziale (prima di vedere i dati) — PRIOR (blu)
...
```{r}
# Conoscenza aggiornata (dopo aver visto i dati) — POSTERIOR (viola)
ggplot(data.frame(x = c(0, 1)), aes(x = x)) +
  stat_function(
    fun  = dbeta,
    args = list(shape1 = alpha_post, shape2 = beta_post),
    color = modern_palette$purple,
    linewidth = 1.1
  ) +
  labs(x = "Probabilità di successo (p)", y = "Densità")
```
...
```{r}
# Previsioni per i prossimi 10 lanci — Posterior predictive (istogramma verde)
ggplot(data.frame(proporzioni = proporzioni_future), aes(x = proporzioni)) +
  geom_histogram(
    aes(y = ..density..),
    bins = 20,
    fill = modern_palette$green,
