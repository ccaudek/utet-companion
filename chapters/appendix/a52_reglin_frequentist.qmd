# Leggere una regressione in forma frequentista {#sec-apx-reglin-frequentist}

::: {.callout-note appearance="minimal"}
Il Companion sviluppa la regressione in chiave bayesiana, e la Parte *Diagnosi* spiega perché il valore-$p$, gli intervalli di confidenza e le soglie di significatività siano strumenti fragili. Resta però un fatto: la quasi totalità della letteratura psicologica pubblicata riporta regressioni in forma frequentista, con tabelle che elencano coefficienti, errori standard, statistiche $t$ e valori-$p$.

Questa appendice serve a leggere quelle tabelle. Non propone un percorso alternativo a quello del Companion, e non va intesa come un'introduzione più semplice al modello lineare: per quella ci sono i capitoli della Parte *Modelli*. Serve a sapere che cosa i numeri di un articolo significano davvero, come sono stati ottenuti e dove un lettore critico deve fermarsi.
:::

::: {.callout-tip title="Domanda chiave"}
Davanti alla tabella di regressione di un articolo, che cosa dice ciascuna colonna — e che cosa non dice?
:::

## Che cosa contiene questa appendice {.unnumbered .unlisted}

- Il modello lineare nella formulazione frequentista, con la distinzione fra parametri fissi della popolazione e stimatori campionari.
- Il metodo dei minimi quadrati e il significato di ciò che minimizza.
- L'errore standard della regressione e l'indice di determinazione $R^2$.
- La diagnostica dei residui.
- L'inferenza sui coefficienti: distribuzione campionaria, errore standard, test e intervalli di confidenza.
- Una guida alla lettura di una tabella di regressione pubblicata.

::: {.callout-tip collapse="true" title="Prerequisiti"}
- Il @sec-linear-generative, per il modello lineare come rappresentazione generativa.
- L'@sec-apx-lin-func, per un ripasso sulla funzione lineare.
- Per il quadro frequentista generale — stime, intervalli, test — il modulo [Inferenza frequentista](https://ccaudek.github.io/utet-frequentista/).
:::

::: {.callout-caution collapse="true" title="Preparazione del notebook"}

```{r}
#| label: setup-reglin-frequentist
#| output: false
...
## Che cosa la regressione può e non può fare

Le finalità della regressione — previsione, descrizione di associazioni, estrapolazione, inferenza causale — sono discusse nel @sec-linear-generative, che distingue le tre domande e mostra perché non ricevano risposta dallo stesso coefficiente. Le conseguenze dell'omissione di variabili rilevanti sono trattate nel @sec-omitted-variable.

Quelle discussioni valgono identiche qui: cambiano gli strumenti inferenziali, non la logica di ciò che un coefficiente può sostenere. Un coefficiente OLS e un coefficiente bayesiano descrivono la stessa quantità — la variazione attesa nell'esito per unità di predittore, condizionatamente agli altri termini del modello — e nessuno dei due diventa causale per virtù del metodo di stima.
...
## Leggere una tabella di regressione pubblicata

Un articolo riporta di norma una tabella con quattro colonne. Ecco che cosa dice ciascuna, e che cosa non dice.

| Colonna | Che cosa riporta | Che cosa **non** significa |
|---|---|---|
| Stima ($b$, $\hat\beta$) | La variazione attesa nell'esito per unità di predittore, condizionatamente agli altri termini | L'effetto di un intervento, salvo che il disegno lo giustifichi |
| Errore standard | La dispersione dello stimatore in ipotetiche repliche del campionamento | L'incertezza sul valore del parametro nella popolazione |
| $t$ e valore-$p$ | Quanto la stima sia distante da zero, in unità di errore standard, e la probabilità di un risultato altrettanto estremo se il parametro valesse zero | La probabilità che l'ipotesi nulla sia vera, né l'importanza dell'effetto |
| Intervallo di confidenza | Un intervallo costruito con una procedura che, ripetuta, conterrebbe il parametro nel 95% dei casi | Che ci sia il 95% di probabilità che il parametro cada in *questo* intervallo |

Alle stesse tabelle si applicano tre domande che il Companion pone sistematicamente.

**Quanto è precisa la stima, non solo se supera la soglia.** Un coefficiente significativo con un errore standard grande è compatibile con effetti di ampiezza molto diversa. Un intervallo che va da 0.05 a 4.2 non sostiene la stessa conclusione di uno che va da 1.9 a 2.3, anche se entrambi escludono lo zero.

**Su quale scala è misurato l'effetto.** Un coefficiente ha significato solo in rapporto all'unità del predittore e alla variabilità dell'esito. Senza deviazioni standard o unità dichiarate, «$b = 0.35$» non è interpretabile.

**Quante analisi sono state svolte prima di questa.** Il valore-$p$ riportato descrive la procedura dichiarata, non il percorso effettivo che ha portato a quella tabella. È la questione dei gradi di libertà del ricercatore, trattata nel @sec-replication-degrees-of-freedom.

## Riflessioni conclusive {.unnumbered .unlisted}

L'appendice ha presentato la regressione lineare bivariata nella formulazione frequentista: il metodo dei minimi quadrati per ottenere stime puntuali dei parametri fissi, e la descrizione della loro precisione attraverso distribuzioni campionarie, errori standard, test e intervalli di confidenza, con gli strumenti diagnostici per valutare le assunzioni.

Il modello statistico sottostante è lo stesso della versione bayesiana: cambia il modo di rappresentare l'incertezza. Nella formulazione frequentista i parametri sono quantità fisse e la casualità riguarda gli stimatori; in quella bayesiana l'incertezza sui parametri è espressa da una distribuzione di probabilità, e questo consente di rispondere direttamente a domande come «quanto è plausibile che la pendenza sia compresa fra $a$ e $b$?».

Conoscere entrambe le formulazioni non serve a scegliere fra due scuole. Serve a leggere la letteratura esistente per quello che dice — e a riconoscere, nelle tabelle che si incontrano ogni giorno, il confine fra ciò che i dati sostengono e ciò che l'abitudine interpretativa vi aggiunge.
