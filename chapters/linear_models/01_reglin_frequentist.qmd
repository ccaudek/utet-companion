...
* la rettadeve minimizzare la somma dei quadrati dei residui (*SSE*).
...
Il coefficiente $a$ indica l'intercetta della retta di regressione nel diagramma a dispersione. Questo valore rappresenta il punto in cui la retta di regressione interseca l'asse $y$ del sistema di assi cartesiani. Come abbimo già detto,  in questo caso specifico, il valore di $a$ non è di particolare interesse psicologico. Successivamente, vedremo come è possibile trasformare i dati per fornire un'interpretazione utile del coefficiente $a$.
...
La stima dei coefficienti di regressione è solo il primo passo dell'inferenza. L'obiettivo principale dell'inferenza è quantificare l'incertezza associata a queste stime, che deriva dalla natura campionaria dei dati. A tale scopo, l'approccio frequentista definisce la **distribuzione campionaria** degli stimatori. 

Nel modello di regressione bivariata:
$
Y_i = \beta_0 + \beta_1 X_i + \varepsilon_i,
$
il metodo dei Minimi Quadrati Ordinari (OLS) fornisce gli **stimatori** $\hat{\beta}_0$ e $\hat{\beta}_1$ che, applicati ai dati a nostra disposizione, producono le stime puntuali dei parametri incogniti $\beta_0$ e $\beta_1$. 

La prospettiva frequentista considera gli stimatori come **variabili casuali**. Il loro valore non è fisso, ma fluttua in funzione del campione casuale estratto: campioni diversi, sebbene provenienti dalla stessa popolazione, producono stime diverse del coefficiente $b_1$.

Il valore $b_1$ ottenuto dal nostro specifico dataset è quindi una singola realizzazione di questa variabile casuale. L'idea centrale è quella di immaginare di ripetere l'intero studio infinite volte: ogni volta si estrae un nuovo campione casuale di dimensione n dalla popolazione, si raccolgono i dati e si ricalcola $b_1$. L'esito di ogni ripetizione sarebbe leggermente diverso. Questa fluttuazione è **l'incertezza campionaria**, una proprietà intrinseca del processo di inferenza da un campione alla popolazione.

La collezione teorica di tutti i possibili valori di $b_1$ ottenuti da queste infinite replicazioni ideali forma la **distribuzione campionaria** di $b_1$. È questa distribuzione che quantifica la nostra incertezza e all'approccio frequentista di costruire intervalli di confidenza e test di ipotesi.

### Le assunzioni del modello e la distribuzione dello stimatore

Per poter fare inferenza sul coefficiente di regressione nella popolazione ($\beta_1$) a partire dalla stima campionaria ($\hat{\beta}_1$ o $b_1$) è necessario definire le proprietà statistiche dello stimatore. Lo stimatore OLS per la pendenza nel modello bivariato è

$$
\hat{\beta}_1 = \frac{\sum_{i=1}^n (X_i - \bar{X})(Y_i - \bar{Y})}{\sum_{i=1}^n (X_i - \bar{X})^2}.
$$

Poiché il suo valore dipende dai dati specifici del campione, $\hat{\beta}_1$ è una **variabile casuale**. Per comprenderne il comportamento e garantire che le nostre inferenze siano valide, dobbiamo fare riferimento a precise assunzioni sul modello.

**1. Le assunzioni di Gauss-Markov.**

Le seguenti condizioni, note come assunzioni di Gauss-Markov, sono sufficienti a garantire che lo stimatore OLS sia il **migliore stimatore lineare corretto** (BLUE, *Best Linear Unbiased Estimator*):
...
4.  **omoschedasticità**: la varianza dell'errore è costante per ogni valore di $X$: $\operatorname{Var}(\varepsilon_i \mid X_i) = \sigma^2$.
5.  **variabilità del predittore**: la variabile $X$ non è costante: $\sum_{i=1}^n (X_i - \bar{X})^2 > 0$.

Per l'inferenza sui piccoli campioni, è necessario fare un'ulteriore assunzione:

*   **normalità degli errori**: $\varepsilon_i \mid X_i \sim N(0, \sigma^2)$.

**2. La distribuzione campionaria di $\hat{\beta}_1$.**

Sotto le assunzioni di Gauss-Markov (e, per l'inferenza esatta, sotto la normalità), la distribuzione campionaria di $\hat{\beta}_1$ ha le seguenti proprietà fondamentali:

*   **correttezza (non distorsione)**: il valore atteso dello stimatore coincide con il parametro vero della popolazione: $\operatorname{E}(\hat{\beta}_1) = \beta_1$. Ciò significa che, se ripetessimo lo studio infinite volte, la media delle stime ottenute sarebbe il valore "vero".
*   **varianza**: la variabilità dello stimatore attorno al suo valore atteso è data da
    ...
    $$
    Poiché $\sigma^2$ è sconosciuta, la si stima con la **varianza residua** $s_e^2 = \frac{\sum e_i^2}{n-2}$. L'**errore standard** di $\hat{\beta}_1$, che misura la precisione della stima, è quindi:
    $$
    \operatorname{SE}(\hat{\beta}_1) = \sqrt{\frac{s_e^2}{\sum (X_i - \bar{X})^2}}.
    $$
*   **forma della distribuzione**:
    *   se gli errori sono normalmente distribuiti, $\hat{\beta}_1$ segue esattamente una distribuzione normale per qualsiasi dimensione campionaria $n$.
    *   se gli errori non sono normali, per grandi campioni ($n$ sufficientemente elevato) $\hat{\beta}_1$ è approssimativamente normale grazie al Teorema del Limite Centrale.

**3. L'utilizzo pratico: l'inferenza statistica**

La conoscenza della distribuzione campionaria di $\hat{\beta}_1$ è ciò che rende possibile l'inferenza statistica:

*   **test di ipotesi**: per verificare, ad esempio, l'ipotesi nulla $H_0: \beta_1 = 0$ (nessuna relazione lineare), si costruisce la statistica test $t = \frac{\hat{\beta}_1}{\operatorname{SE}(\hat{\beta}_1)}$, la cui distribuzione è nota sotto $H_0$.
*   **intervalli di confidenza**: si costruisce un intervallo attorno alla stima puntuale, come $\hat{\beta}_1 \pm t_{n-2,\,0.975} \times \operatorname{SE}(\hat{\beta}_1)$, che fornisce un range di valori plausibili per il parametro $\beta_1$ nella popolazione.

In sintesi, la distribuzione campionaria di $\hat{\beta}_1$ descrive la variabilità che ci si aspetta di osservare nella stima del coefficiente angolare se si ripetesse lo studio molte volte su campioni diversi. Comprenderne le proprietà, ovvero la posizione (correttezza), la dispersione (errore standard) e la forma (normale asintotica), è essenziale per interpretare i risultati di una regressione frequentista, valutare la significatività delle relazioni e trarre conclusioni valide sulla popolazione di riferimento.[^caudekluccio]
...
In questo capitolo abbiamo presentato la regressione lineare bivariata secondo la prospettiva frequentista, illustrando come il metodo dei minimi quadrati permetta di stimare i parametri della retta e di quantificarne l'incertezza inferenziale. Abbiamo approfondito il ruolo dell'intercetta, della pendenza e della varianza residua, sottolineando le condizioni necessarie affinché il modello costituisca una sintesi affidabile dei dati.

Questa cornice classica, pur solida e ampiamente adottata, presenta tuttavia dei limiti significativi. L'inferenza si concentra principalmente suo valori-$p$ e sugli intervalli di confidenza, non consente di incorporare conoscenze pregresse e richiede assunzioni rigide, come la normalità degli errori. Questo approccio è efficace per descrivere le associazioni, ma è poco flessibile e talvolta distante dalle domande sostanziali della ricerca scientifica.

Per superare questi vincoli, nei prossimi capitoli esploreremo la regressione lineare da una prospettiva alternativa: quella **bayesiana**. Questo approccio permette di integrare in modo esplicito le informazioni a priori, di rappresentare tutta l'incertezza in termini di distribuzioni di probabilità e di ottenere stime più ricche e direttamente interpretabili. Questo sarà il primo passo per elevare la regressione da semplice strumento descrittivo a un modello inferenziale più ampio e aderente alla complessità tipica dei dati psicologici.
