...
Nel manuale abbiamo analizzato i fondamenti teorici della critica al valore-$p$: la sua definizione come probabilità condizionata all'ipotesi nulla, le controversie storiche tra Fisher e Neyman-Pearson e il celebre monito dell'ASA del 2016 [@wasserstein2016asa].
...
**Da sapere.** Il valore-$p$ esprime la probabilità dei dati assumendo vera l’ipotesi nulla, non la probabilità che l’ipotesi nulla sia vera. Il $p$-hacking e l’uso rigido della soglia 0.05 possono produrre conclusioni instabili e aumentare i falsi positivi.

**Da saper fare.** Eseguire e interpretare simulazioni che mostrano l’inflazione dei falsi positivi, le anomalie nella distribuzione dei valori-$p$, la confusione tra $P(\text{Dati} \mid H_0)$ e $P(H_0 \mid \text{Dati})$ e la fragilità delle decisioni basate su una soglia fissa.

**Approfondimento facoltativo.** Le evidenze tratte dalla letteratura, i passaggi analitici più dettagliati e gli esercizi finali approfondiscono gli stessi problemi, ma possono essere saltati senza perdere il filo concettuale del capitolo.
...
tibble(p_value = best_p_values) |>
  ...
  scale_color_manual(values = c("Soglia α = 0.05" = palette_qualitative[6]))         # Rosso-arancio
```
...
p1 <- tibble(p_value = honest_pvalues) |>
  ...
  scale_color_manual(values = c("Soglia α = 0.05" = palette_qualitative[6])) 
```

```{r}
# Grafico B: Letteratura con p-hacking
